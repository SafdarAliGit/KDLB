...
class Party(TransactionBase):
	...
	def get_customer_name(self):
		...
		if frappe.db.get_value("Party", self.customer_name) and not frappe.flags.in_import:
			count = frappe.db.sql(
				"""select ifnull(MAX(CAST(SUBSTRING_INDEX(name, ' ', -1) AS UNSIGNED)), 0) from tabParty
				 where name like %s""",
	...
	def validate(self):
		...
		if frappe.db.exists("Party", self.name):
			customer = frappe.get_doc("Party", self.name)
	...
	def get_customer_group_details(self):
		doc = frappe.get_doc("Party Group", self.customer_group)
		self.accounts = []
		self.credit_limits = []
	...
	def check_customer_group_change(self):
		...
		if not self.get("__islocal"):
			if self.customer_group != frappe.db.get_value("Party", self.name, "customer_group"):
	...
	def validate_internal_customer(self):
		...
		if internal_customer:
			frappe.throw(
				_("Internal Party for company {0} already exists").format(
	...
	def update_customer_groups(self):
		...
		if frappe.flags.customer_group_changed:
			update_linked_doctypes(
				"Party", self.name, "Party Group", self.customer_group, ignore_doctypes
			)
	...
	def update_lead_status(self):
		"""If Party created from Lead, update lead status to "Converted"
		update Party link in Quotation, Opportunity"""
		if self.lead_name:
	...
	def link_lead_address_and_contact(self):
		if self.lead_name:
			...
			for row in linked_contacts_and_addresses:
				...
				if not linked_doc.has_link("Party", self.name):
					linked_doc.append("links", dict(link_doctype="Party", link_name=self.name))
	...
	def validate_name_with_customer_group(self):
		if frappe.db.exists("Party Group", self.name):
			frappe.throw(
				_(
					"A Party Group exists with same name please change the Party name or rename the Party Group"
				),
				frappe.NameError,
			)
	...
	def on_trash(self):
		...
		delete_contact_and_address("Party", self.name)
		if self.lead_name:
			frappe.db.sql("update `tabLead` set status='Interested' where name=%s", self.lead_name)

	def after_rename(self, olddn, newdn, merge=False):
		if frappe.defaults.get_global_default("cust_master_name") == "Party Name":
	...
	def set_loyalty_program(self):
		...
		if len(loyalty_program) == 1:
			...
			frappe.msgprint(
				_("Multiple Loyalty Programs found for Party {}. Please select manually.").format(
...
def make_quotation(source_name, target_doc=None):
	...
	target_doc = get_mapped_doc(
		"Party",
		source_name,
		{"Party": {"doctype": "Quotation", "field_map": {"name": "party_name"}}},
		target_doc,
		set_missing_values,
	)

	target_doc.quotation_to = "Party"
	target_doc.run_method("set_missing_values")
	target_doc.run_method("set_other_charges")
	target_doc.run_method("calculate_taxes_and_totals")

	price_list, currency = frappe.db.get_value(
		"Party", {"name": source_name}, ["default_price_list", "default_currency"]
	)
	if price_list:
...
def get_loyalty_programs(doc):
	...
	for loyalty_program in loyalty_programs:
		if (
			...
			in get_nested_links(
				"Party Group", loyalty_program.customer_group, doc.flags.ignore_permissions
			)
		) and (
...
def get_customer_outstanding(
	...
	outstanding_based_on_gle = frappe.db.sql(
		...
		from `tabGL Entry` where party_type = 'Party'
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = frappe.db.get_value(
			"Party Credit Limit",
			{"parent": customer, "parenttype": "Party", "company": company},
			"credit_limit",
		)

		if not credit_limit:
			customer_group = frappe.get_cached_value("Party", customer, "customer_group")
			credit_limit = frappe.db.get_value(
				"Party Credit Limit",
				{"parent": customer_group, "parenttype": "Party Group", "company": company},
				"credit_limit",
			)
